# curl Command-Line Usage Reference

A concise reference for implementing a curl-compatible command-line tool in C++.

## URL Handling

### Supported Schemes
HTTPS, HTTP, WSS, WS, SMTPS, SMTP, IMAPS, IMAP, SFTP, FTPS, FTP, SCP, POP3S, POP3, LDAPS, LDAP, SMBS, SMB, MQTT, RTSP, RTMP, FILE, TELNET, TFTP, GOPHERS, GOPHER, DICT

### URL Format
- Scheme separator: `://` (accept 1-3 slashes)
- Without scheme: guess from hostname prefix (ftp., smtp., etc.) or default to HTTP
...
- `--ssl`: try TLS upgrade (FTP, etc.)
- `--ssl-reqd`: require TLS upgrade
...
### Mode
- `--use-ascii` / `-B`: ASCII transfer mode
- Default: binary mode

### Directory Operations
- URL ending in `/`: directory listing
- `-l`, `--list-only`: names only (NLST)
- `--ftp-method multicwd|nocwd|singlecwd`

### Connection Mode
- Default: PASV (passive)
- `-P`, `--ftp-port ADDR`: PORT (active)
- `--ftp-pasv`: explicit passive
- `--no-epsv`, `--no-eprt`: disable extended commands
- `--ftp-skip-pasv-ip`: ignore PASV IP

### FTPS
- `ftps://` URL: implicit TLS
- `--ssl` or `--ssl-reqd`: explicit TLS (STARTTLS)

### Custom Commands
- `-Q`, `--quote CMD`: before transfer
- `-Q -CMD`: after transfer
- `-Q +CMD`: after CWD, before transfer
- `-Q "*CMD"`: allow failure
...
- `--mail-rcpt ADDR`: recipient (repeatable)
- `-T FILE`: message body
- URL path: HELO/EHLO hostname

### IMAP
- `imap://server/MAILBOX;UID=N`: get by UID
- `imap://server/MAILBOX;MAILINDEX=N`: get by index

### POP3
- `pop3://server/N`: get message N
- `--request DELE pop3://...`: delete

---

## SCP/SFTP

- User required: `sftp://user@host/path`
- Home directory: `/~/path`
- `--key FILE`: private key
